...
class eNetMusic extends ListenerAdapter {
    ...
    /**
     * Creates and runs the eNetMusic bot with the given bot token and bot management role arguments.
     *
    ...
    public static void main(String[] args) {
        String token = "";
        String botManagementRole = "";
        try {
            if (args.length >= 2) {
                token = args[0];
                botManagementRole = args[1];
            }
            else {
            ...
            new JDABuilder()
                    ...
                    .setBotToken(token)
                    .addListener(new eNetMusic(botManagementRole))
                    .buildBlocking();
        }
    ...
    /**
     * MusicPlayer object used to add and play AudioSources.
    ...
    /**
     * URL for AudioSource the player will used.
     * @see eNetMusic#player
     */
    private String audioLink = "";

    /**
     * Tells the bot if audioLink is a radio station or YouTube/SoundCloud link.
     * @see eNetMusic#player
     */
     ...
     * @see net.dv8tion.jda.entities.VoiceChannel
     */
    ...
    /**
     * Sets the bot management role and pre-loads some radio stations into radioStations.
     *
     * radioStations = HashMap of two Strings
     * First string is the radio station name, second string is the mp3 stream link.
     *
     * @param botManagementRole The bot's management role (used for sending command errors if any occur).
     */
    private eNetMusic(String botManagementRole){
        // Set the bot's management role.
        managerRole = botManagementRole;
    ...
    private void playRadio(TextChannel channel, String commandline){
        ...
        if(commandline.toLowerCase().startsWith("/radio"))
            commandline = "/play -radio" + commandline.substring(("/radio").length());
            ...
            else
            {
                ...
                if (info.getError() == null)
                {
                    ...
                    if (!(commandline.toLowerCase().contains("youtube.com") || commandline.toLowerCase().contains("youtu.be")
                        ...
                        if(InternetRadioParser.getStationName(audioLink).toLowerCase().contains("could not connect to: ")) {
                            for(String s : radioStations.keySet()) {
                            ...
                            msg = "**" + InternetRadioParser.getStationName(audioLink) + "** has been queued";
                    }
                    else {
        ...
        for (int i = 0; i < queue.size() && i < 10; i++)
        {
            ...
            else
            {
                ...
                if(!(audioSource.toLowerCase().contains("youtube.com") || audioSource.toLowerCase().contains("youtu.be")
                    ...
                    if(InternetRadioParser.getStationName(audioSource).toLowerCase().contains("could not connect to: "))
                        for (String s : radioStations.keySet()) {
                    ...
                    if(!unreadableRadio)
                        builder.appendString("]` " + InternetRadioParser.getStationName(audioSource) + "\n");
                }
    ...
    private void nowPlaying(MusicPlayer player, TextChannel channel){
        if (player.isPlaying())
        {
            ...
            if(playerType.equalsIgnoreCase("radio")){
                ...
                if(!(InternetRadioParser.getStationName(audioSource).equalsIgnoreCase(audioSource)
                        && InternetRadioParser.getCurrentSong(audioSource).equalsIgnoreCase(audioSource))
                        && !unreadableRadio)
                    channel.sendMessage(
                            "**Playing:** " + InternetRadioParser.getStationName(audioSource) + "\n" +
                                    "**Current Song:** " + InternetRadioParser.getCurrentSong(audioSource));
        ...
        if(commandline.substring(("/remove").length()).length() > 1) {
            ...
            if (oldSize == player.getAudioQueue().size())
                ...
                if (playerType.equalsIgnoreCase("youtube") || playerType.equalsIgnoreCase("soundcloud"))
                    ...
                    String radioName = "";
                    boolean unreadableRadio = false;
                    if (InternetRadioParser.getStationName(removedSong.getSource()).toLowerCase().contains("could not connect to: ")) {
                    ...
                    if (!(InternetRadioParser.getStationName(removedSong.getSource()).equalsIgnoreCase(removedSong.getSource())
                            && InternetRadioParser.getCurrentSong(removedSong.getSource()).equalsIgnoreCase(removedSong.getSource()))
                            && !unreadableRadio)
                        channel.sendMessage("Removed **" + InternetRadioParser.getStationName(removedSong.getSource()) + "** from the queue!");
                    else if (unreadableRadio)
    ...
    private void pausePlayer(MusicPlayer player, TextChannel channel){
        if(player.isPaused())
            channel.sendMessage("Playback is already paused!");
        else {
            player.pause();
